...
gen_gen <- function(spi,rs,N,alpha_mat,model,obs_error_dist,...){
  ...
  } else if (names(rs) != rownames(alpha_mat) | names(N) != rownames(alpha_mat)){
    stop("'rs' and 'N' names must match the row names of 'alpha_mat'")
  } else if (missing(obs_error_dist) == FALSE & obs_error_dist %in% c("normal","poisson","nb") == FALSE){
  ...
  if (model == "LV"){
    fecund <- (rs %*% (1-comp_coef))[spi]
  } else if (model == "BH"){
    fecund <- (rs %*% 1/(1+comp_coef))[spi]
  } else if (model == "R"){
    fecund <- (rs %*% exp(-comp_coef))[spi]
  }
  if (missing(obs_error_dist)){
    return(fecund)
  } else if (obs_error_dist == "normal"){
    return(rnorm(n=1, mean=fecund, sigma=...))
