...
# working on a generating function that can write brms-compatible functions with nl parameters,
# for any number of species and for any choice of dynamics

bfwriter <- function(nspec,dynam){
  alphas <- list("aaa","aab","aac","aad","aae","aaf","aag","aah","aai","aaj","aak","aal","aam","aan")
  ins <- list("Na","Nb","Nc","Nd","Ne","Nf","Ng","Nh","Ni","Nj","Nk","Nl","Nm","Nn")
  if (dynam == "LV"){
    eq <- "formula=as.formula(y~lami*(1"
    for (i in 1:nspec){
      eq <- paste(eq,"-",alphas[[i]],"*",ins[[i]],sep="")
    }
    eq <- paste(eq,"))",sep="")
    nlps <- ",nl=TRUE,lami~1"
    for (i in 1:nspec){
      nlps <- paste(nlps,",",alphas[[i]],"~1",sep="")
    }
